...
public class TreatyUpdateProcessor {
    ...
    private void update(String title, TreatyChangeEvent event) {
        ...
        body.append("Time remaining: " + DiscordUtil.timestamp(existing.getEndTime(), null) + "\n");
        body.append("From: " + DNS.getMarkdownUrl(existing.getFromId(), true));
        if (fromAA != null) {
            maxRank = fromAA.getRank();
            body.append(" - #" + maxRank + " (" + fromAA.getNations(true, 7200, true).size() + " nations)");
        }
        body.append("\n");
        body.append("To: " + DNS.getMarkdownUrl(existing.getToId(), true));
        if (toAA != null) {

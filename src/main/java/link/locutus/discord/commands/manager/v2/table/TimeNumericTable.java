...
public abstract class TimeNumericTable<T> {
    ...
    public static TimeNumericTable metricByGroup(Set<NationAttributeDouble> metrics, Set<DBNation> coalitionNations, @Default("getCities") NationAttributeDouble groupBy, @Switch("i") boolean includeInactives, @Switch("a") boolean includeApplicants, @Switch("t") boolean total) {
        coalitionNations.removeIf(f -> f.isVacation() || (!includeApplicants && f.getPosition() <= 1) || (!includeInactives && f.active_m() > 4880));
...
//        SimpleNationList allNationsList = new SimpleNationList(allNations);
    ...
    public TimeNumericTable<T> convertHoursToEpochSeconds(long hourStart) {
        for (int i = 0; i < data.getRowCount(); i++) {
            Row row = data.getRow(i);
            long hour = hourStart + ((Number) row.get(0)).longValue();
            long time = TimeUtil.getTimeFromHour(hour) / 1000L;
            data.set(0, i, time);
        }
        return this;
    }

...
public class Activity {
    ...
    double[] byDayTurn = new double[24];
    double[] byWeekTurn = new double[24 * 7];
    ...
    public Map.Entry<Integer, double[]> optimalAttackTime(int numTurns, boolean week) {
        ...
        int dayTurn = now.getHour();
        int weekTurn = dayTurn + day.ordinal() * 24;
    ...
    private void load(Set<Long> activity, long minTurnAbs, long maxTurnAbs) {
        ...
        if (maxTurnAbs == Long.MAX_VALUE) maxTurnAbs = TimeUtil.getHour();

        int[] byDayTotal = new int[7];
        int[] byDayTurnTotal = new int[24];
        int[] byWeekTurnTotal = new int[24 * 7];
        ...
        for (long turn = minTurnAbs; turn <= maxTurnAbs; turn++) {
            ...
            ZonedDateTime other = now.minusHours(turnDiff);

            DayOfWeek day = other.getDayOfWeek();
            int dayTurn = other.getHour();
            int weekTurn = dayTurn + day.ordinal() * 24;
